...
test('notification list renders seeded jobs masked and links to a detail page', async ({ page }) => {
  ...
  // A sent email job is seeded in the default scenario. Below the wide-table
  // breakpoint a card list renders; assert against its visible content.
  await expect(page.locator('.notification-list__card').filter({ hasText: 'Sent' }).first()).toBeVisible()
  ...
  // Navigate directly to the seeded sent job's detail (deterministic route).
  await page.goto(`/demo/notifications/${JOB_ID_PAID_SENT}`)
  await expect(page.getByRole('heading', { name: 'Notification detail', level: 1 })).toBeVisible()
})
...
test('dead -> confirmation -> retry -> sent (operational retry flow)', async ({ page }) => {
  ...
  await page.goto(`/demo/notifications/${JOB_ID_MANUAL_RETRY}`)
  ...
  await expect(page.getByTestId('attempt-error-code').first()).toBeVisible()
